/**
*
* @license Guriddo jqGrid JS - v5.5.4 - 2021-02-16
	...
	from : function(source){
		...
		this.startsWith=function(f,v){
			...
			if(_useProperties){
				...
				if (v!=null) { length=_trim?$.jgrid.trim(v.toString()).length:v.toString().length; }
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var title = $(ts.p.caption).text() ? $(ts.p.caption).text() : ts.p.caption;
		$(this).attr({role:"grid","aria-multiselectable":!!this.p.multiselect,"aria-labelledby":"gbox_"+this.id, title:title});
		...
		formatCol = function (pos, rowInd, tv, rawObject, rowId, rdata){
			...
			} else if ($.jgrid.isFunction(cm.cellattr) || (typeof cm.cellattr === "string" && $.jgrid.cellattr != null && $.jgrid.isFunction($.jgrid.cellattr[cm.cellattr]))) {
		...
		constructTr = function(id, hide, classes, rd, cur ) {
			...
			if( typeof rowAttrObj !== "object" ) {
				rowAttrObj = $.jgrid.isFunction(ts.p.rowattr) ? ts.p.rowattr.call(ts, rd, cur, id) :
					(typeof ts.p.rowattr === "string" && $.jgrid.rowattr != null && $.jgrid.isFunction($.jgrid.rowattr[ts.p.rowattr]) ?
		...
		imgs += "<span sort='asc'  class='ui-grid-ico-sort ui-icon-asc"+iac+" ui-sort-"+dir+" "+disabled+" " + iconbase + " " + getstyle(stylemodule, 'icon_asc', true)+ "'" + sort + ">empty</span>";
		imgs += "<span sort='desc' class='ui-grid-ico-sort ui-icon-desc"+idc+" ui-sort-"+dir+" "+disabled+" " + iconbase + " " + getstyle(stylemodule, 'icon_desc', true)+"'" + sort + ">novalue</span></span>";
		if(ts.p.multiSort) {
			if(ts.p.sortname ) {
		...
		firstr = null;
		var title = $(ts.p.caption).text() ? $(ts.p.caption).text() : ts.p.caption;
		var hTable = $("<table "+getstyle(stylemodule,'headerTable',false,'ui-jqgrid-htable ui-common-table')+" style='width:"+ts.p.tblwidth+"px' role='grid' aria-labelledby='gbox_"+this.id+"' title='"+title+"'></table>").append(thead),
...
$.jgrid.extend({
	...
	setGridWidth : function(nwidth, shrink, usefrozen = true) {
		return this.each(function(){
			if (!this.grid ) {return;}
	...
	setGridHeight : function (nh, entrie_grid = false, usefrozen= true) {
		return this.each(function (){
			var $t = this;
			if(!$t.grid) {return;}
			...
			if( maxfrozen>=0 && frozen) {
				...
				if( $($t).jqGrid('isGroupHeaderOn') )  {
					...
					$("tr.jqg-first-row-header, tr.jqg-third-row-header", htbl).each(function(){
						$("th", this).slice( maxfrozen + 1 ).remove();
					});
				...
				$($t).on('jqGridAfterGridComplete.setFrozenColumns', function () {
					...
					$("tr[role=row]",btbl).each(function(){
						$("td[role=gridcell]", this).slice( maxfrozen + 1).remove();
					});
					...
					if($t.p.headerrow) {
						...
						$("tr",hrtbl).each(function(){
							$("td", this).slice( maxfrozen + 1 ).remove();
						});
						$(hrtbl).width(1);
